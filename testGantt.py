...
def localize_activities(activities):
    ...
    return activities_with_loc

def localize_timestamps(timeline):
    """
    Add location information to primary timestamps by searching for them in the PDF.
    Only the primary timestamps (most granular) are used for horizontal alignment.
    """
...
with pdfplumber.open(path3) as pdf:
    ...
    image_path = helper.convert_pdf2img(path3)
    tables = first_page.extract_table()
    boxes = first_page.rects
    #print(boxes)
    ...
    print("With AI")
    print(timeline)
    time_line_with_localization = localize_timestamps(timeline)
    activities_with_loc = localize_activities(activities)
    gantt_chart_bars = find_bars(boxes, activities_with_loc,2)
    #visualize_with_matplotlib(path2, gantt_chart_bars, activities_with_loc)
    matcher = match_bars_with_timeline(gantt_chart_bars,time_line_with_localization)
    #print(matcher)
