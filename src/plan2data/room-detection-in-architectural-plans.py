...
import numpy as np
#https://github.com/MjdMahasneh/room-detection-in-architectural-plans/blob/master/main.py
...
def detect_rooms(img, background_label, spaces_label):
    ...
    #All pixels less than white → 255 (white in the mask)
    # White background pixels → 0
    # helps to separate lines/ walls form background 

    # Erode the image Helps connect broken walls → so that rooms will later appear as closed contours.
    kernel = np.ones((5, 5), np.uint8)
    eroded_img = cv2.erode(binary_mask, kernel, iterations=2)

    # Detect edges in the eroded image 
    # Canny Edge Detection → finds edges in the eroded image.
    # Thresholds: 50 (low), 150 (high)
    edges = cv2.Canny(eroded_img, 50, 150)

    # Detect lines using the Hough Line Transform
    #Detects straight lines in edges
...
if __name__ == '__main__':
    ...
    img = cv2.imread("examples/FloorplansAndSectionViews/bemasster-grundriss-plankopf_page1.png", cv2.IMREAD_GRAYSCALE)
    ...
    for label in range(0, num_labels):  # Skip background (label 0)
        ...
        # Extract pixels
        pixels = rooms_mask[:, :, 0][y:y + h, x:x + w][mask]

        # Safe access
        if pixels.size > 0:
            original_value = pixels[0]
        else:
            original_value = 0  # default fallback: assume 'floor' or 'space'
        ...
        if not np.isnan(cx) and not np.isnan(cy):
            ...
            cv2.putText(output_img, text, (int(cx), int(cy)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)
