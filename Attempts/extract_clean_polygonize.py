...
def extract_lines_and_text(pdf_path, min_length_pts=141.75):
    """Extract line segments and text from a PDF using PyMuPDF.

    Args:
        pdf_path (str): Path to the PDF file.
        min_length_pts (float): Minimum line length in PDF points to keep (default ~5cm).

    Returns:
        tuple: (lines, texts) where lines is a list of dicts {start, end, length}
               and texts is a list of dicts {text, bbox}.
    """
...
def clean_lines(lines, min_length_pts=141.75):
    """Filter out short or degenerate lines.
...
def polygonize_lines(lines):
    """Convert line segments to Shapely polygons using polygonize.

    Args:
        lines (list): List of line dicts with `start` and `end` tuples.

    Returns:
        list: Polygons as lists of (x, y) coordinates.
    """
    line_strings = []
    for l in lines:
        s = l.get("start")
...
def run_cli():
    ...
    parser.add_argument("--min-length", type=float, default=141.75,
                        help="Minimum line length in PDF points to keep (default ~141.75 pts ~= 5cm)")
